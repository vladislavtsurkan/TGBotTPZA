    ...
    async with state.proxy() as data:
        match data_inline_keyboard:
            case 'group', 'change_title':
                await callback.message.edit_text('Введіть нову назву для групи', reply_markup=None)
                await FSMEditGroup.input_edit_title.set()
            case 'group', 'change_department':
                await callback.message.edit_text('Введіть назву нової кафедри для групи', reply_markup=None)
                await FSMEditGroup.input_edit_department.set()
            case 'group', 'change_url':
                await callback.message.edit_text('Тепер відправте посилання на розклад групи з '
...
def register_handlers_fsm_edit_group(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(group_edit_callback, Text(startswith='group'),
                                       state=FSMEditGroup.select_edit_group)
    dp.register_message_handler(input_new_title_for_edit_group, state=FSMEditGroup.input_edit_title)
    dp.register_message_handler(input_new_url_schedule_for_edit_group, state=FSMEditGroup.input_edit_schedule_url)
    dp.register_message_handler(input_new_department_for_edit_group, state=FSMEditGroup.input_edit_department)
