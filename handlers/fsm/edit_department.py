...
from services.admin import (
    get_department_instance_by_title, change_faculty_for_department, delete_department, change_title_for_department
)
    ...
    async with state.proxy() as data:
        match data_inline_keyboard:
            ...
            case 'department', 'change_faculty':
                await callback.message.edit_text('Введіть назву нового факультету для кафедри.', reply_markup=None)
...
def register_handlers_fsm_edit_department(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(department_edit_callback, Text(startswith='department'),
                                       state=FSMEditDepartment.select_edit_department)
    dp.register_message_handler(input_new_title_for_edit_department, state=FSMEditDepartment.input_edit_title)
    dp.register_message_handler(
        input_new_title_short_for_edit_department, state=FSMEditDepartment.input_edit_title_short
    )
    dp.register_message_handler(input_new_faculty_for_edit_department, state=FSMEditDepartment.input_edit_faculty)
